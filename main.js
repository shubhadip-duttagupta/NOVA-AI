...
//API SETUP
const API_KEY = "AIzaSyDQUyu5Seq8digKm7Y4msziV89W1633d84";
...
// Stimulate tuping effect for bot responses 
...
// Make API call and generate the bot's responce
const generateResponse = async (botMsgDiv) =>{
    const textElement = botMsgDiv.querySelector(".message-text");
    controller = new AbortController();

// Add user message and file data in the chat history
    chatHistory.push({
        role: "user",
    ...
    try{
        ...
        textElement.textContent = error.name === "AbortError" ? "Responce generation stopped" : error.message;
        botMsgDiv.classList.remove("loading");
        document.body.classList.remove("bot-responding");
    }finally{
        userData.file = {};
...
//generate user message HTML and add in the chtas container
...
document.querySelector("#stop-responce-btn").addEventListener("click", () =>{
    userData.file = {};
    controller?.abort();
    clearInterval(typingInterval);
